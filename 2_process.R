...
# - add retractions and expressions of concern
# - add citations
# June 2026
library(readxl)
...
source('0_my_openalex_key_do_not_share.R') # my key
...
for (k in 1:N) {
  ...
  data$country[k] = list(add_country(
    countries,
    states,
    text = data$aff[k][[1]]
  )) # authors' countries
...
## get citations from OpenAlex in batches
batch_size = 100 # number of DOIs to get from OpenAlex per request
# split the DOIs into batches
doi_batches <- split(data$doi, ceiling(seq_along(data$doi) / batch_size))
max_batch = length(doi_batches)
open_alex = NULL
re_start = 1161 # if re-starting due to 429 error
for (b in re_start:max_batch) {
  from_oa = oa_fetch(
    doi = doi_batches[[b]],
    entity = 'works',
    abstract = FALSE,
    options = list(select = c('doi', 'cited_by_count')),
    api_key = my_open_alex_key,
    mailto = 'a.barnett@qut.edu.au'
  )
  Sys.sleep(5) # to avoid "429 Too Many Requests"
  open_alex = bind_rows(open_alex, from_oa)
  from_oa = NULL # safety net
}
# safety net for duplicates in case of any restarts
open_alex = unique(open_alex)
...
open_alex = group_by(open_alex, doi) %>%
  ...
  ungroup()
# add OpenAlex citation data to original data
...
data = mutate(
  data,
  time_between = as.numeric(accepted - received),
...
# Retractions from ‘Annulled’ Detector (https://dbrech.irit.fr/pls/apex/f?p=9999:33)
# Expressions of concern from Problematic Paper Screener
# downloaded on 29 July 2026 & 8 August 2026; restricted to PLOS publisher
concern = read.csv('data/concerning_2026_08_08.csv') %>%
  clean_names()
annulled = read.csv('data/annulled_2026_08_08.csv') %>%
  clean_names()
annulled = bind_rows(annulled, concern, .id = 'ret_type') %>%
  separate(decision, into = c('date', 'reason'), sep = ' ') %>% # all reasons are 'retracted' or expression of concern
  mutate(
    ret_type = ifelse(ret_type == 1, 'retracted', 'concern'),
  ...
  select(
    ...
    -doctype,
    -citations,
    -reason, # all retracted or concern
    -altmetric
  ) %>% # not needed
  rename('ret_date' = 'date')
# just use one if retraction and concern
annulled = group_by(annulled, doi) %>%
  arrange(ret_date) %>%
  slice(1) %>%
  ungroup()
censor.date = as.Date(Sys.Date()) # date annulled database searched
# add to main data
data = left_join(data, annulled, by = 'doi')
data = mutate(
  data,
  retracted = case_when(
    # binary retracted variable (cannot be part of previous merge)
    is.na(ret_date) == FALSE ~ TRUE,
    is.na(ret_date) == TRUE ~ FALSE
  ),
  time_to_retraction = case_when(
...
prop.table(table(data$retracted)) * 100 # check percentage retracted / concern

# check for duplicates
table(table(data$doi)) # should all be 1
get_dupes(data)
# slim down data
data = select(data, -aff, -domain, -'journal', -venue, -accepted, -ret_date) # no longer needed
