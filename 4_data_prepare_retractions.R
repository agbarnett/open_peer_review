...
data = mutate(
  ...
  ret_type = ifelse(is.na(ret_type), 'none', ret_type), # replace missing
  time_to_retraction = time_to_retraction / 365.25
) # convert to years
...
data <- mutate(
  ...
  p_orcid = p_orcid, # best fractional polynomial = linear
  ...
  published = sqrt(published), # best fractional polynomial, square root
  time_between = sqrt(time_between + 1) # square-root transformation
)
