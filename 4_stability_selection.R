...
stab.lasso <- stabsel(
  ...
  PFER = 1
) # per-family error rate; expected number of falsely selected variables

# refit model with selected variables (no shrinkage)
index = colnames(x) %in% attr(stab.lasso$selected, 'names')
x_selected = x[, index]
...
for (this_name in categorical) {
  which_col = which(str_detect(
    colnames(x),
    pattern = paste(this_name, '$', sep = '')
  )) # because of Health_care_...
  if (length(which_col) != 1) {
    (cat('Error finding column for', this_name, '\n', sep = ''))
  }
...
save(
  ...
  stab.lasso,
  small_model,
  small_model_logistic,
  x_selected_names,
  file = 'results/4_ests_stability.RData'
)
