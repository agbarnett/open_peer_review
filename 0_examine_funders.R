...
numbers = unlist(data$funder_number_consolidated) %>%
  str_remove(
    'https://data.crossref.org:443/fundingdata/funder|http://data.crossref.org/fundingdata/funder/10.13039/|https?://(dx.)?doi.org/10.13039/|https?://doi.org/10.3030|https?://dx.doi.org'
  ) %>%
  str_remove('\\/')
...
tab = mutate(tab, numbers = as.character(numbers))

# loop through all funders (takes a few minutes)
funder_text = NULL
re_start = 648
for (k in re_start:N) {
  # takes a little while due to web searches
  ...
  if (length(main_name) > 1) {
    cat('Multiple names for ', number, '\n', sep = '')
  }
  ...
  if (str_detect(main_name, ' and ')) {
    # to cover common use of ampersand
  ...
  all_names = all_names[
    !str_detect(all_names, '^(The )?Department (of|Of) Health$')
  ...
  all_names = paste(
    '\\b',
    paste(all_names, collapse = '\\b|\\b'),
    '\\b',
    sep = ''
  ) # paste all together; add breaks to reduce false positives
  ...
  # concatenate data
  frame = data.frame(
    funder_number = as.character(number),
    lower_case = all_names
  ) #
  funder_text = bind_rows(funder_text, frame)
  # reset
  all_names = NA
  Sys.sleep(1) # avoid timeouts from crossref
  # progress
  if (k %% 500 == 0) {
    cat('Up to ', k, '\n', sep = '')
  }
...
funder_text = mutate(
  funder_text,
  lower_case = str_replace(
    lower_case,
    pattern = 'cooperation gmbh\\)',
    replacement = 'cooperation gmbh'
  )
)
...
bland = c('ministry of education\\\\', 'ministry of health and welfare\\\\')
...
## part 2: check duplicates in funder names
