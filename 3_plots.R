...
tplot = ggplot(data = types, aes(x = x, y = estimate))+
  ...
  g.theme
tplot
ggsave('figures/3_type.jpeg', tplot, width=4, height=4, units='in', dpi = 500)
...
splot = ggplot(data = subjects, aes(x = x, y = estimate))+
  ...
  g.theme
splot
ggsave('figures/3_subjects.jpeg', splot, width=4, height=8, units='in', dpi = 500)
...
cplot = ggplot(data = countries, aes(x = x, y = estimate))+
  ...
  g.theme
cplot
ggsave('figures/3_country.jpeg', cplot, width=4, height=4, units='in', dpi = 500)
...
hplot
ggsave('figures/3_bootstrap_elastic_net_variable_numbers.jpeg', hplot, width=4, height=4, units='in', dpi = 500)
...
tplot = ggplot(data = what_t, aes(x=df, y=WAIC))+
  ...
  ylab('WAIC')
tplot
ggsave('figures/3_best_t_distribution.jpeg', tplot, width=4.2, height=3.7, units='in', dpi = 500)

# save the list of candidates for use in the Bayesian model
save(candidates, what_t, file = 'results/3_candidates.RData')
