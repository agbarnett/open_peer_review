...
# plot the categorical estimates from the stability selection with an outcome of open peer review
# August 2026
library(stringr)
library(dplyr)
library(ggplot2)
...
source('R/funder_name.R') # for funder names
...
# load the results
load('results/4_ests_stability.RData') # from 4_stability_selection.R on HPC
...
for_plot = filter(for_plot, stab.lasso.max >= 0.25) %>%
  mutate(x = 1:n(), selected = rowname %in% selected)
# labels
...
labels = str_replace_all(labels, 'funder_', 'Funder = ')
for (k in 1:length(labels)) {
  labels[k] = funder_name_single(labels[k]) # see function
}
...
pplot = ggplot(
  data = for_plot,
  aes(x = x, y = stab.lasso.max, col = selected)
) +
  geom_point(size = 3) +
  scale_x_continuous(
...
# export
ggsave(
  'figures/5_stability_selection.jpg',
  pplot,
  width = 4.9,
  height = 5.2,
  units = 'in',
...
cat(
  'There were ',
  filter(for_plot, stab.lasso.max == 1) %>% nrow(),
  ' variables that were selected in all 500 bootstrap samples.\n',
  sep = ''
)
...
to_plot = filter(ests, !str_detect(term, 'Intercept|published$|orcid')) %>%
  mutate(
    term = str_remove(term, '^x_selected'),
    ...
    term = funder_name(term),
    # add narrower intervals - did not use
...
cplot = ggplot(
  data = to_plot,
  aes(x = x, y = estimate, ymin = conf.low, ymax = conf.high, col = group)
) +
  geom_point(size = 3) +
  ...
  scale_x_continuous(
    breaks = 1:nrow(to_plot),
    labels = to_plot$term,
    expand = expansion(mult = 0, add = expand)
  ) + # using add for consistent gap by panels
  g.theme +
  theme(
    legend.position = 'none',
    strip.text.x = element_text(
      margin = margin(t = 0.7, r = 0, b = 0.7, l = 0, "mm")
    )
  ) + # reduce facet size
  coord_flip()
# now make column version ...
cplot_column = cplot +
  ggforce::facet_col(vars(group), scales = 'free', space = 'free') # forced into one column
# export
ggsave(
  'figures/5_stability_estimates.jpg',
  cplot_column,
  width = 5.8,
  height = 7.2,
  units = 'in',
...
# ... and square version (did not use)
expand = 0.6
cplot_square = cplot +
...
ggsave(
  'figures/5_stability_estimates_square.jpg',
  cplot_square,
  width = 7.2,
  height = 7.2,
  units = 'in',
...
# export
ggsave(
  'figures/5_stability_country.jpg',
  cplot2,
  width = 4.7,
  height = 4.5,
  units = 'in',
...
cplot3 = ggplot(
  ...
  scale_color_manual(NULL, values = cbbPalette) +
  scale_x_continuous(
    breaks = 1:nrow(to_plot_not_country),
    labels = to_plot_not_country$term,
    expand = c(expand, expand)
  ) +
  g.theme +
  theme(legend.position = 'none', plot.margin = unit(c(0, 0.3, 0, 0), "cm")) + # avoid cut-off at right side
  coord_flip() +
...
to_plot_subject = filter(to_plot, group == 'Subject') %>%
  mutate(
    term = str_replace(term, "environmental", "environ-\nmental"), # to squeeze in
    x = 1:n()
  )
splot = ggplot(
  data = to_plot_subject,
  aes(x = x, y = estimate, ymin = conf.low, ymax = conf.high)
) +
  ggtitle('Subject') +
  geom_point(size = 2, color = colour) +
...
to_plot_funder = filter(to_plot, group == 'Funder') %>%
  mutate(x = 1:n(), term = str_replace(term, ' Research ', '\nResearch\n')) # to reduce white space around funder name
fplot = ggplot(
  data = to_plot_funder,
  aes(x = x, y = estimate, ymin = conf.low, ymax = conf.high)
) +
...
lmat = matrix(
  c(1, 2, 2, 3, 3, 3, 3, 3, 3, 6, 4, 4, 4, 4, 4, 4, 4, 4, 5, 6),
  ncol = 2,
  byrow = FALSE
)
...
grid.arrange(
  ...
  layout_matrix = lmat,
  heights = lheights
) # heights for x-axis label at bottom
dev.off()

# alternative version with consistent scales (version that was used in paper)
...
jpeg(
  'figures/5_stability_estimates_consistent.jpg',
  width = 7.2,
  height = 7.2,
  units = 'in',
  res = 500
