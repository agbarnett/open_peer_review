...
* 0_funder_info.RData, meta-data on funders
* 0_unprocessed.RData, after reading in the XML data for the full PLOS corpus
* 2_reviews.RData, randomly selected reviews
* 2_processed.RData, processed data that includes citations and retractions, and authors' countries
* 3_plus_experience.RData, further processed by adding author's experience from _OpenAlex_
...
* funderNames.csv, funder ID numbers and names
* Sentiment_Analysis_may5.xlsx, Sentiment analysis comparing reviews from PLOS Medicine and BMC Medicine
...
* n_orcids = number of authors with an ORCID (integer)
* subject = articles subjects (list format) 
* n_emails = number of author emails given (integer) 
* no_conflict = no conflicts of interest (TRUE/FALSE) 
...
* edomain = email domain (list format)
...
* funder_number_consolidated = funder number (list)
* author_papers = author's paper numbers at submission (integer)
* atype = which author was used for author_papers (character)
