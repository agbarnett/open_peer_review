...
# DAG
dag_e <- dagify(
  #open ~ journal, # openness will depend on journal -- turned off, assume all effects happen via topic
...
for_plot = tidy_dagitty(dag_e) %>%
  mutate(
    dependent = str_detect(name, pattern = '^open'),
...
dag_plot = ggplot(
  for_plot,
  aes(x = x, y = y, xend = xend, yend = yend, col = dependent, shape = latent)
) +
...
## retractions
# time to retraction depends on open peer review
dag_c <- dagify(
  open ~ good, # openness depends on a good review
  open ~ topic, # openness will depend on topic
  open ~ country, # openness will depend on country
  open ~ quality, # openness depends on solid high-quality research
  robust ~ quality, # robust methods follows from quality research
  good ~ robust, # robust methods lead to better reviews
  retraction ~ topic, # retraction depends on field/topic
  retraction ~ country, # retraction depends on country
  retraction ~ quality, # retraction depends on high-quality research
  labels = c(
    "open" = "Open peer\nreview",
    "topic" = "Topics",
    "country" = "Author's\ncountry",
  ...
  latent = c('quality', 'good', 'robust'),
  exposure = "open",
  outcome = "retraction"
)
# get data and plot to allow unmeasured variables in different shape
for_plot_retraction = tidy_dagitty(dag_c) %>%
  mutate(
    dependent = str_detect(name, pattern = '^retraction'),
    latent = str_detect(name, pattern = 'good|quality|robust')
  )
# plot
dag_plot_retraction = ggplot(
  for_plot_retraction,
  aes(x = x, y = y, xend = xend, yend = yend, col = dependent, shape = latent)
) +
  geom_dag_point(show.legend = FALSE) + # stops both legends
...
# export
ggsave(
  filename = 'figures/99_dag_retraction.jpg',
  plot = dag_plot_retraction,
  width = 5,
  height = 4,
  units = 'in',
  dpi = 400
