...
pos[pos=='4_data_prepare'] = 3.5 # in-between 3 and 4
pos = as.numeric(as.factor(pos)) # re-order
pos = as.numeric(table(pos))
pos = c(3,pos) # add three for data sources (crossref, etc)
...
M[which(files=='4_survival_retractions'), which(files=='4_prepare_data')] = ' ' # no data, but a link
M[which(files=='4_citations'), which(files=='4_prepare_data')] = ' ' # no data, but a link
...
M[which(files=='5_plots.R'), which(files=='4_stability_selection')] = paste("'", data[5], "'", sep='') # needs to be ests results file
