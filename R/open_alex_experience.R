...
get_alex_experience = function(dois, dates, oa_key){
  ...
  if(is.null(paper_data)){
    cat('Sleep for 429.\n')
    Sys.sleep(60*3) # to avoid rate after 429 error; sleep for 3 mins
    paper_data = oa_fetch(entity = 'works',
                          abstract = FALSE,
                          mailto = 'a.barnett@qut.edu.au', 
                          api_key = oa_key,
  ...
  if(lengths(paper_data)[1] < n_input){
    ...
    fframe = data.frame(doi = missing_dois, atype = 'Missing DOI', author_papers = NA)
  ...
  for (paper_loop in 1:n_input){
    ...
    if(is.na(id) == TRUE){ # if first missing, use random author
      random_author = filter(these_authors, !is.na(id)) # only those with an ID
	  if(nrow(random_author) == 0){
		fframe = data.frame(doi = this_doi, atype = 'No authors with an OpenAlex ID', author_papers = NA)
		all_data = bind_rows(all_data, fframe)
		next
	  }
      random_author = random_author %>% sample_n(1)
    ...
    if(is.null(result)){
      cat('Sleep for 429 at author stage.\n')
      Sys.sleep(5) # to avoid rate after 429 error; sleep for 5 seconds
      result = tryCatch(oa_request(query,
                                   api_key = oa_key,
                                   mailto = 'a.barnett@qut.edu.au'), error = function(e) { NULL })
    } 
