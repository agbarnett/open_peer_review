...
# add names
load('data/0_funder_info.RData') # from 0_examine_funders.R
...
# check for duplicate funder names from list
funder_text = mutate(
  funder_text,
  lower_case = str_remove_all(lower_case, '\\\\b')
)
group_by(funder_text, lower_case) %>% slice(2)
